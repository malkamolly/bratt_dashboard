...
export default async function PricingSection() {
  ...
  return (
    <>
      <div className="mb-2 mt-10 rounded-md bg-status-warn/40 px-4 py-2 text-xs font-extrabold uppercase tracking-wide text-ink">
        Draft — pricing isn&apos;t finalized. A starting point to react to, not a rate sheet.
      </div>

      {/* ---------- The calculator ---------- */}
      <Card title="Price any tree (up to 100″)">
        <p className="mb-4 max-w-3xl text-sm text-fg-2">
          Enter DBH (trunk diameter), height, and crown spread. Price = DBH × the
          base rate for that size, adjusted up or down for a tree that&apos;s taller
          or wider than typical.
        </p>
        <PriceCalculator />
      ...
      <Card title="How much do height & spread move the price?" className="mt-8 border-orange">
        <p className="mb-4 max-w-3xl text-sm text-fg-2">
          From <strong>{analyzed.toLocaleString()}</strong> real removals, with the
          trunk-size effect removed so height and spread show on their own. Each bar
          is how much more (or less) per inch a tree cost versus a{' '}
          <strong>typical</strong> tree of its size. Both matter, but less than the
          calculator&apos;s flat <strong>{effect.modelStepPct}%</strong> assumption
          — the data says about <strong>{effect.heightStepPct}%</strong> per band for
          height and <strong>{effect.canopyStepPct}%</strong> for spread.
        </p>
        ...
        <p className="mt-3 text-xs text-fg-3">
          Bars with a paler fill sit on fewer than 10 jobs — read the top bands as
      ...
      <Card title="New pricing vs. what we charged" className="mt-8">
        <p className="mb-4 max-w-3xl text-sm text-fg-2">
          ...
          within a few percent of what we billed, while the big trees (34″+) are now
          priced meaningfully higher — exactly the change we made.
        </p>
