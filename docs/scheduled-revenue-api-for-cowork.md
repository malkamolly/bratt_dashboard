...
In ServiceTitan, run the saved report **(Claude) Scheduled Revenue** with these
parameters — they're what the current export was built from:

| Parameter | Value |
|---|---|
| Date Range | `01/01/26 – 01/01/30` (a wide window; the calendar only shows what's there) |
| Filter by | `Jobs with Appt Date` |
| Business Unit | `All` |
| Include Adjustment Invoices | `false` |
...
`Location Zip`. The first five plus `Jobs Subtotal` and `Scheduled Date` are
required; the rest are optional and degrade to blank.

The export ends with a **grand-total row** — no status, no business unit, no
date, the row count in `Job #`, and the summed subtotal. Don't send it as a job.
Send its two numbers as the checksum instead.
...
| `jobs` | Required, non-empty. **Every row** from the report except the grand-total row, including $0 jobs — the checksum is validated before anything is filtered. |
...
Both fields are required and both are enforced before anything is persisted.

- `rowCount` vs `jobs.length`
- `subtotalSum` vs the summed `subtotal` values (compared in cents, one cent of
  tolerance for float summation)

Any mismatch → **422**, nothing persisted, and the message names the failing
check with both expected and computed values.

**Take both from the export's own grand-total row.** This is the whole reason
the JSON path is safer than uploading a file: a half-read spreadsheet has
nothing to fail against and just quietly imports a lighter November, whereas a
half-read extraction fails this check and is refused.
...
optional field `sourceDate`. This is what the browser uploader uses. The file
path reads the export's own grand-total row and validates against it, so it
gets the same protection the JSON checksum gives.
...
```json
{
  ...
  "onTheBoard": { "revenue": 1332782.31, "jobs": 749 },
  "next7":      { "revenue": 133594.62,  "jobs": 120 },
  "next30":     { "revenue": 377113.81,  "jobs": 309 },
  "next90":     { "revenue": 975320.88,  "jobs": 678 },
  ...
  "byMonth": [
    { "month": "2026-09", "revenue": 377113.81, "jobs": 309, "holdRevenue": 7317 }
  ],

  "nextWeeks": [
    { "weekOf": "2026-08-31", "revenue": 167349.35, "jobs": 123 },
    { "weekOf": "2026-09-07", "revenue": 70974.57,  "jobs": 72 }
  ],
...
Send the same `sourceDate` (today in Central) on both runs.
...
| 413 | Body over the cap (2 MB JSON, 4 MB file). A day's board is ~280 KB as JSON. |
| 415 | Content-type is neither `application/json` nor `multipart/form-data`. |
| 422 | Schema violation, bad date, checksum mismatch, or an export with no jobs in it. The message names the field or check. |
...
select created_at, endpoint, outcome, status_code,
       source_date, job_count, firm_revenue, reason
