...
export type Bucket = 'needs_reply' | 'waiting' | 'handled' | 'fyi';
...
export async function buildBoard(ownerEmail: string): Promise<TriageBoard> {
  ...
  for (const key of ['needs_reply', 'waiting', 'handled', 'fyi'] as const) {
...
async function cardFor(
  ...
  if (bucket === 'waiting') {
    ...
    if (mine?.text) card.userLastLine = mine.text;
  }
