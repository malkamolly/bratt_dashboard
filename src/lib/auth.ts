...
/**
 * Can this role upload a new Job Completed Detail export to refresh the
 * collections list?
 *
 * Same reasoning as the Follow-Up Scorecard: an upload REPLACES the report for
 * everyone, so it stays with the people who run collections. Mirrors the RLS
 * write policy in migration 074_receivables.sql.
 */
export function canUploadReceivables(role: Role): boolean {
  return role === 'admin' || role === 'sales_manager';
}

/**
 * Can this role import new Scheduled Revenue exports (the revenue calendar at
 * /production/revenue-calendar)?
 *
 * An import REPLACES the calendar for everyone, so it stays with the people who
